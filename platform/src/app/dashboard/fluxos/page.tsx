...
function Transitions({ draft, version, flows, updateVersion }: { draft: FlowView; version: FlowVersionView; flows: FlowView[]; updateVersion: (patch: Partial<FlowVersionView>) => void }) {
  return <fieldset><legend className="text-sm font-semibold text-slate-ink">Próximos fluxos permitidos</legend><p className="mt-1 text-xs text-stone">O servidor rejeita destinos não selecionados nesta versão.</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{flows.filter((flow) => flow.key !== draft.key).map((flow) => { const checked = version.allowedTransitions.includes(flow.key); return <label key={flow.key} className="flex items-start gap-3 rounded-lg border border-mist bg-white p-3 text-sm"><input type="checkbox" checked={checked} onChange={() => updateVersion({ allowedTransitions: checked ? version.allowedTransitions.filter((key) => key !== flow.key) : [...version.allowedTransitions, flow.key] })} className="mt-0.5 accent-deep-teal" /><span><strong className="text-slate-ink">{flow.name}</strong><span className="mt-1 block font-mono text-[11px] text-stone">{flow.key}</span></span></label>; })}</div></fieldset>;
}
