...
export function PanelHeader({
  ...
  return (
    <header className="flex min-w-0 flex-col items-stretch gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div className="flex min-w-0 items-start gap-3">
      ...
      {action ? (
        <Link href={action.href} className="inline-flex self-start items-center gap-1 text-xs font-semibold text-deep-teal hover:text-forest-teal sm:shrink-0">
          {action.label}<ArrowRight className="h-3.5 w-3.5" />
        </Link>
      ) : meta ? <p className="self-start text-xs font-semibold text-stone sm:shrink-0">{meta}</p> : null}
    </header>
  );
}